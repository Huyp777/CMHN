...
class DataSet:
    def __init__(self, t):
        ...
        videos = h5py.File(f"{DATASET_DIR_ACTIVITYNET}sub_activitynet_v1-3.c3d.hdf5", "r", libver='latest', swmr=True)
        sentences = pickle.load(open(f'{DATASET_DIR_ACTIVITYNET}activitynet_captions_bert_25_{t}.pkl', 'rb'))
        self.names = list(sentences.keys())
        for name in tqdm(self.names):
            the_feature_v = videos[name]['c3d_features'][()]
            sample = the_feature_v.shape[0] / sentences[name]['duration']
            self.feature_v[name] = np.stack([utils.tools.normalize(np.mean(the_feature_v[int(i - sample):max(int(i - sample) + 1, int(i))], axis=0)) for i in np.arange(sample, len(the_feature_v) + sample, sample)]).astype(np.float32)
            ...
            for i, timestamps in enumerate(sentences[name]['timestamps']):
                timestamp0 = np.array(min(timestamps[0], timestamps[1]))
                timestamp1 = np.array(max(timestamps[0], timestamps[1]))
...
def train():
    ...
    for epoch in range(0, EPOCHS):
        ...
        print(res)
        if res[3] > max_performance:
            max_performance = res[3]
